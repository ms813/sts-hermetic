...
public class CoagulateActions {

    private static final Logger logger = LoggerFactory.getLogger(CoagulateActions.class);
    ...
    public AlchemyTuple apply(
        ...
        logger.info("Applying coagulate action: ae={} s={}", ae, s);

        if (s == 0 && ae == 0) {
            logger.info("Attempted to perform a coagulate action but no essences were on the target!");
        }
        // rows starting east of the diagonal
        else if (s == 0 && ae > 0) {  // first row
            new CoagulatePureAetherAction().accept(tuple, target, source);
        ...
        //columns starting south of the diagonal
            ...
            new CoagulatePureSanguisAction().accept(tuple, target, source);
    ...
    public AurumPower apply(
        ...
        logger.info("Nothing implemented for coagulate aurum yet {}", aurum.amount);
